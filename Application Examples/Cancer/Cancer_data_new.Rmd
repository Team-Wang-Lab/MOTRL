...
# did not use this
```{r}
label(CancerData.table1$`Age at Reg`) = "Age (years)" 
label(CancerData.table1$`Def Loc Rx?`)= "History of ???"
label(CancerData.table1$`Time, Horm Rx to Reg (mo)`) = "Time of horm??? before registration"
label(CancerData.table1$Strat) = "Disease Volume"
label(CancerData.table1$`Alk Phos`) = "Alk Phos ???"
label(CancerData.table1$Hgb) = "Hgb???"
label(CancerData.table1$`Bsln PSA`) = "PSA at Baseline (ng/mL)" 
label(CancerData.table1$PSA1) = "PSA after first chemo (ng/mL)"
label(CancerData.table1$`Treatment 1`) = "First chemotherapy"
label(CancerData.table1$`Treatment 2`) = "Second chemotherapy"
label(CancerData.table1$`Exp.score1` )= "Expert score after first chemotherapy"
label(CancerData.table1$`Exp.score2`) = "Expert score after second chemotherapy"
label(CancerData.table1$`Surv from Reg (mo)`) = "Survival time after registration (mo)"

table1(~ `Age at Reg` + `Def Loc Rx?` + `Time, Horm Rx to Reg (mo)` + Strat + `Alk Phos` + Hgb + `Bsln PSA` + PSA1 + `Treatment 1`+ `Treatment 2` + `Exp.score1` + `Exp.score2` + `Surv from Reg (mo)` , render.continuous = my.render.cont, render.categorical = my.render.cat, data = CancerData.table1, overall=F, extra.col=list(`P-value`=pvalue))
```
...
```{r}
Cov2.cor = Cov2
levels(Cov2.cor$`Def Loc Rx?`) = c(0,1)
levels(Cov2.cor$Strat) = c(1,0)
Cov2.cor$`Def Loc Rx?` = as.numeric(Cov2.cor$`Def Loc Rx?`)
Cov2.cor$Strat = as.numeric(Cov2.cor$Strat)
Cov2.cor = as.matrix(Cov2.cor)
#create matrix of correlation coefficients and p-values
# rcorr(Cov2.cor)
cor2pcor(cor(Cov2.cor)) # ??? Baseline PSA and PSA2 have colinarity...
# pcor(Cov2.cor, method = "pearson")
```
...
tree2_MOtol3.4 = MO.tol.DTRtree(Ys = Ys2, w = w2.3, A = Trt2, H = cbind(Cov2,Y1.3),
                              delta = 0.4, tolerate = TRUE, m.method="AIPW", 
                              depth = 3, lambda.pct = 0.0001, minsplit = 2)
knitr::kable(tree2_MOtol3.4$tree, digits = 2)
```
