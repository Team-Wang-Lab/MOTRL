#  A simulation example for 1 stages and 3 treatments scenario
...
# 1. function to summarize simulation results (whether the regimes includes the optimal regimes in each models)
...
# Simulation begin
set.seed(1)
x1<-rnorm(N)              # each covaraites follows N(0,1)
x2<-rnorm(N)
x3<-rnorm(N)
x4<-rnorm(N)
x5<-rnorm(N)
X0<-cbind(x1,x2,x3,x4,x5) # All of the covariates
