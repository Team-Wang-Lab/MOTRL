...
The goal of MOTRL is to directly estimate optimal dynamic treatment regime (DTR) or tolerant DTR (tDTR) in a multi-stage multi-treatment setting. The user can self-define their preference on different priorities and the tolerant rate at each stages.
...
Then, calculate the cummulated mean pseudo outcomes, and use it as the outcomes (`Ys`).
The 
```{r}
